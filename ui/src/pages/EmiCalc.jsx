...
function EmiCalc() {
  const [loan, setLoan] = useState(loanType[1]);
 ...
 const handleSubmit = (e) => {
  ...
  return (
    <div className="bg-teal-100">
      ...
      <div>
        <div className="flex min-h-full flex-1 lg:w-[35rem] w-[18rem] m-auto mt-10 rounded-xl shadow-2xl shadow-teal-900 border-teal-900  bg-gray-100 flex-col justify-center px-6 py-12 lg:px-8">
          <h3 className="text-center text-2xl mb-10 text-blue-600 font-bold">
            EMI Calculator
          </h3>
          <form action="#" method="POST" className="space-y-6 h-[30rem]">
            <Listbox value={loan} onChange={(value) => {
              setLoan(value);
              ...
              <div className="relative mt-2">
                <ListboxButton className="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pr-2 pl-3 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
                  <span className="col-start-1 row-start-1 flex items-center gap-3 pr-6">
                    <span className="block truncate">{loanType.name}</span>
                  </span>
                  <ChevronUpDownIcon
                    aria-hidden="true"
            ...
            <div>
            <Listbox value={loanCode} onChange={(value) => {
              setCode(value);
